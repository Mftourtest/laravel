...
@section('content')
    <style>
        ...
        .list-group{margin: 0;}
        .modal{top: 15%;}
        .modal-detail-body{text-align: center;}
        .modal-detail-footer{color: red;}
        ...
        .coupontext {color: #00a0dc;font-size:2rem;}
        ...
        .btn-1 {position: absolute;top: 2.5rem;right: 1rem;background-color: white;}
        a .btn-1 {color: #00a0dc;font-size: 1rem; border: 1px solid; margin-top: -35px}
    </style>
        ...
        <div class="tab-content" style="position: relative; padding-top: 40px; background: ghostwhite;">
            <div class="tab-pane active" id="good">
                <div class="row">
                    ...
                    <div class="col-xs-9">
                                ...
                                <div class="tab-pane {{ $i==0?'active':'' }}" id="cate_{{ $category->id }}">
                                    <ul class="list-group" style="margin-bottom: 40px;">
                                        ...
                                        @foreach($category->foods as $j=>$food)
                                            <li class="list-group-item">
                                                <img class="detail" src="{{ $cdn.$food->thumb }}" alt="{{ $food->id }}" style="width: 60px; height: 60px;">
                                                ...
                                                <div style="position: absolute; right: 1rem; bottom: 1rem;">
                                                    @if(!empty($food->pack))
                                                        <button class="btn btn-xs btn-blue pack" data-toggle="modal" data-target=".modal-packages" data-title="{{ $food['title'.$suffix] }}" style="padding: 0.2rem 1rem; ">
                                                            @lang('foods.biz_order_title1')
                                                        </button>
                        ...
                        {{--模态框--}}
                        <div class="modal fade bs-example-modal-sm modal-packages">
                            <div class="modal-dialog modal-sm" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                        <h4 class="modal-title modal-pack-title">菜品名称</h4>
                                    </div>
                                    <div class="modal-body modal-pack-body"></div>
                        ...
                        <div class="modal fade bs-example-modal-sm  modal-detail">
                            <div class="modal-dialog modal-sm" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                        <h4 class="modal-title modal-detail-title"></h4>
                                    </div>
                                    <div class="modal-body modal-detail-body"></div>
                                    <div class="modal-footer modal-detail-footer"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <form action="{{ route('foods.confirm', ['param'=>$param]) }}" method="post" enctype="application/x-www-form-urlencoded">
                            <div class="content">
                                <div class="content-left">
                                    ...
                                    <div class="price">{{ $cate->money }} <span>0</span></div>
                                </div>
            ...
            <div role="tabpanel" class="tab-pane" id="comment">
                <ul class="list-group group_list">
                    @if(empty($comments[0]))
                        <li class="list-group-itemtwo comment" style="text-align: center; color: #aaa;">还没有人评论哦</li>
                    @endif
                    ...
                    @foreach($comments as $comment)
                        <li class="list-group-itemtwo comment">
                            <div class="comment-header">
                                <div>
                                    <img src="{{ $comment->headimgurl ? $comment->headimgurl : url('images/QRcode.jpg') }}">
            ...
            <div role="tabpanel" class="tab-pane" id="partner">
              ...
              <!--  <div style="width: 100%;height: 1rem;background: ghostwhite;"></div>-->
               <div class="tabpanel_list">
                   @foreach($partnerCoupons as $coupon)
                       <div class="activety">
                            <div class="activety_jian">减</div>
                            <span>{{ $coupon->title }}</span>
                       </div>
                   @endforeach
                   @if($partner->discount != 1)
                       <div class="activety">
                            <div class="activety_zhe">折</div>
                            <span>全场商品折扣{{ $partner->discount *10 }}折起</span>
                       </div>
                   @endif
	               {{--<div class="activety" >
...
@push('scripts')
    ...
    <script>
        ...
        $(".detail").on("touchend", function (e) {
            var title = $(this).next().children(":first").html();
            var price = $(this).next().children(":last").html();
            var src = $(this).attr("src");
            var img = '<img src="' + src + '">';
            console.log(title, price, src);

            $(".modal-detail-title").html(title);
            $(".modal-detail-body").html(img);
            $(".modal-detail-footer").html(price);
        ...
        $(".pack").on("click", function (e) {
            $(".modal-pack-body").empty();
            $(".calc2").hide();
            var id = $(this).next().attr("data-id");
            var packages = $(this).next().children();
            //$(".modal-pack-body").append('<input type="hidden" name="fid" value="'+id+'">');
            packages.each(function (i, e) {
                ...
                $(".modal-pack-body").append($label);
            })
            $(".modal-pack-title").html($(this).attr("data-title"));
        });

        // 规格标签点击 label1
        $(".modal-pack-body").on("click", ".label1", function (e) {
            var id = $(this).attr("data-id");
            var price = parseFloat($(this).attr("data-price"));
            var title = $(".modal-pack-title").html() + ' [ ' +$(this).text() +' ]';
            ...
            if ($("#list").find("#food_"+$id).attr("id") == 'food_'+$id) {
                ...
                html =
                    '<li class="list-group-item cart-list" id="food_' + $id + '">\n' +
                        ...
                        '<div class="price">{{ $cate->money }}<span>' + $price + '</span></div>\n' +
