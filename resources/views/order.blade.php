...
<html>
	...
	<body>
		<header class="mui-bar mui-bar-nav" style="background: white;">
			...
			<h1 class="mui-title" style="font-size: 1rem;">@lang('foods.food_index')@lang('foods.biz_order_desk_sn')（{{$desk_sn}}）</h1>
		</header>
		<div class="order">
								...
								<div class="addor">
                                    ...
                                    <div data-id="{{$food['id']}}" style="display: none;">
                                        @foreach($food['packages'] as $pack)
                                            <input type="hidden" data-id="{{ $pack['food_id'].'_'.$pack['id'] }}" data-name="{{ $pack['name'.$suffix] }}" data-price="{{ $pack['price'] }}">
                                        @endforeach
                                    </div>
									</div>
                                @else
