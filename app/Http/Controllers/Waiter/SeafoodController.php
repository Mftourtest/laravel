...
class SeafoodController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('seafood/index')->with("lang",$this->lang)->with('cookies',$cookies);
    }
    ...
    public function login_validator()
    {
        ...
        if($input = Input::all()){
            ...
            if($validator->passes()){  
                ...
                if(!$admininfo){  
                    ...
                    if($psw!=$input['password']){  
                        ...
                        return redirect('seafood/table?lang='.$this->lang); 
                    } 
               }  
            }else{  
            ...
            return view('seafood/index')->with("lang",$lang);;  
        }
    }
    ...
    public function desk_info()
    {
        ...
        //如果session过期就跳回登录页
        if(empty($p_id)){
            return redirect('seafood/index?lang='.$this->lang);
        }
        if($p_id){
        ...
        return view('seafood/table')->with('areainfos',$areainfos)->with('suffix',$this->suffix)
    ...
    public function food_info()
    {
        ...
        foreach($cateinfo as $i=>$cate){
            ...
            $brr = $this->objectToArray($categorys[$i]);
                foreach ( $brr as $j => $food) {
                    ...
                    $data[$i][$j]['is_seafood'] = $food['is_seafood']; //是否属于按斤数算价格的海鲜
                    if($food['pack']==1){                 
        ...
        return view('seafood/order')->with('cateinfo',$arr)->with('data',$data)->with('fooddata',$fooddata)
    ...
    public function placeorder_info()
    {
        ...
        return view('seafood/placeorder')->with('lang',$this->lang)
    ...
    public function place_order()
    {
        ...
        foreach($goods as $i=>$good){
         ...
         return redirect('seafood/orderover?lang='.$this->lang)->with('lang',$this->lang);
         //return view('seafood/orderover')->with('lang',$this->lang);
    ...
    public function non_checkout()
    {
        ...
        return view('seafood/reckoning')->with("lang",$this->lang)->with("enomination",$enomination)
    ...
    public function cash()
    {
        ...
        return view('seafood/cash')->with('lang',$this->lang)->with("enomination",$enomination)
            ->with('desk_sn',$desk_sn)
    ...
    public function coupon()
    {
        ...
        if(!empty($arr)){
          if($coupon_info['consume']=='N' && $now<=$coupon_info['expire_time']){
             ...
             return view('seafood/coupon')->with('lang',$this->lang)->with('orders',$orders)
        ...
        return view('seafood/coupon')->with('lang',$this->lang)->with('desk_sn',$desk_sn)
    ...
    public function wechat()
    {
        $desk_sn = $_GET['desk_sn'];
        $enomination = $_GET['price'];
        return view('seafood/wechat')->with("lang",$this->lang)->with("enomination",$enomination)
            ->with('desk_sn',$desk_sn);
    }
    ...
    public function voucher(Request $request)
    {
        ...
        if(!empty($id)){
            ...
            return view('seafood/voucher')->with("lang",$this->lang)->with("res",$res)
            ->with('desk_sn',$desk_sn);
        }
        else{
            return view('seafood/voucher')->with("lang",$this->lang)->with("voucher_id",$id)
            ->with('desk_sn',$desk_sn);
        }
    }
    ...
    {
        $desk_sn = $_GET['desk_sn'];
        return view('seafood/discount')->with("lang",$this->lang)
            ->with('desk_sn',$desk_sn);
    }
    ...
    public function menu()
    {
        ...
        return view('seafood/menu')->with("lang",$this->lang)->with('desk_sn',$desk_sn)->with('ordertemps',$arr)
